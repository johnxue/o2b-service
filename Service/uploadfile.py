...
class Handler(tornado.web.RequestHandler):
    ...
    def post(self):
        try :
            ...
            codeorid   =objData['codeorid']  # 用户名/产品编码/圈贴ID
            
            # 看是HTML传过来的JSON Key 是否为 picture
            if self.request.files == {} or 'picture' not in self.request.files \
               or attribution not in "header,product.banner,product.large,product.medium,product.small,group" :
            ...
            if attribution=='header':
                ...
                tmp_name='group-'+user+'-'+codeorid+image_format
            else :
                tmp_name='prodcut-'+codeorid+'-'+attribution.split('.').pop().lower()+'.jpg'
                
           
            tmp_name = image_path +'/'+str(int(time.time())) + image_format
            im.save(tmp_name)
            
            file={'filename':tmp_nme}
