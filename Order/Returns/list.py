...
class info(tornado.web.RequestHandler):
    ...
    def get(self):  # 查询用户退换货信息
        ...
        try :
            #1.1 查询退换货单；
            sqlSelect=("SELECT id,swapOrderNo,orderNo,oid,pcode,pname,number,`mode`,description, "
                       "createTime,updateTime,status,comment FROM vwReturns where  user=%s limit %s,%s")
            rows_list=db.query(sqlSelect,(user,offset,rowcount))
    ...
    def post(self):  # 创建退货换单
        ...
        try :
            ...
            sqlInsert = (
              "INSERT INTO tbReturns (user,oid,orderNo,swapOrderNo,pcode,pname,number,mode,description,imgProblem,createUser,createTime,isDelete,status) "
              "VALUES (%s,%s,%s,%s, %s, %s, %s, %s, %s, %s, %s, now(),'N','310')"
            )
